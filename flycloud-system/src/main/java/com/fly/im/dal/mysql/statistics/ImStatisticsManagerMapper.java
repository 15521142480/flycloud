package com.fly.im.dal.mysql.statistics;
...
public interface ImStatisticsManagerMapper {
    ...
    @Select("SELECT COUNT(*) FROM sys_user " +
            "WHERE is_deleted = 0 AND create_time >= #{beginTime} AND create_time < #{endTime}")
    ...
    @Select("SELECT COUNT(DISTINCT user_id) FROM (" +
            ...
            "  WHERE is_deleted = 0 AND " + NORMAL_MESSAGE_CONDITION +
     ...
     * 当前有效群总数
     */
    @Select("SELECT COUNT(*) FROM im_group WHERE is_deleted = 0 AND status = 0")
    ...
    @Select("SELECT " +
            ...
            "  FROM im_group g " +
            "  LEFT JOIN im_group_member m ON m.group_id = g.id AND m.status = 0 AND m.is_deleted = 0 " +
            "  WHERE g.is_deleted = 0 AND g.status = 0 " +
            "  GROUP BY g.id" +
            ") t " +
            "GROUP BY `range`")
