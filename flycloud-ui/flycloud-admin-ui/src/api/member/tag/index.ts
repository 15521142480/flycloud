...
export const getMemberTagPage = async (params: any) => {
  return await request.get({ url: `/${SYS_BASE_URL}/admin/member/tag/page`, params })
}
...
export const getSimpleTagList = async () => {
  return await request.get({ url: `/${SYS_BASE_URL}/admin/member/tag/list-all-simple` })
}

// 新增会员标签
export const createMemberTag = async (data: TagVO) => {
  return await request.post({ url: `/${SYS_BASE_URL}/admin/member/tag/create`, data })
}

// 修改会员标签
export const updateMemberTag = async (data: TagVO) => {
  return await request.put({ url: `/${SYS_BASE_URL}/admin/member/tag/update`, data })
}
