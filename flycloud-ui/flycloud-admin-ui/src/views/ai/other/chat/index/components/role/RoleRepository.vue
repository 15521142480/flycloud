...
import ChatRoleForm from '@/views/ai/other/model/chatRole/ChatRoleForm.vue'
import RoleCategoryList from './RoleCategoryList.vue'
import { ChatRoleApi, ChatRolePageReqVO, ChatRoleVO } from '@/api/ai/other/model/chatRole'
...
const getActiveTabsRole = async () => {
  if (activeTab.value === 'my-role') {
    myRoleParams.pageNum = 1
    await getMyRole()
  } else {
    publicRoleParams.pageNum = 1
    await getPublicRole()
  }
}
...
const handlerCardPage = async (type) => {
  try {
    loading.value = true
    if (type === 'public') {
      publicRoleParams.pageNum++
      await getPublicRole(true)
    } else {
      myRoleParams.pageNum++
