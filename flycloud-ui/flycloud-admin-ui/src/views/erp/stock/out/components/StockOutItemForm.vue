<template>
  ...
  >
    <el-table :data="formData" show-summary :summary-method="getSummaries" class="-mt-10px">
      ...
      <el-table-column
        :label="t('auto.views.erp.stock.out.components.StockOutItemForm.k6f55a5a5')"
        min-width="125"
      >
        <template #default="{ row, $index }">
          ...
          >
            <el-select
              ...
              clearable
              filterable
              :placeholder="t('auto.views.erp.stock.out.components.StockOutItemForm.k7aaa3bbb')"
  ...
  <el-row justify="center" class="mt-3" v-if="!disabled">
    <el-button @click="handleAdd" round>{{ t('extra.k227880c0') }}</el-button>
  </el-row>
</template>
...
const formRules = reactive({
  inId: [
    {
      required: true,
      message: t('auto.views.erp.stock.out.components.StockOutItemForm.k7da936e6'),
      trigger: 'blur'
    }
  ],
  warehouseId: [
    {
      required: true,
      message: t('auto.views.erp.stock.out.components.StockOutItemForm.k8a307436'),
      trigger: 'blur'
    }
  ],
  productId: [
    {
      required: true,
      message: t('auto.views.erp.stock.out.components.StockOutItemForm.k0b3cde2b'),
      trigger: 'blur'
    }
  ],
  count: [
    {
      required: true,
      message: t('auto.views.erp.stock.out.components.StockOutItemForm.k55ce5fbe'),
      trigger: 'blur'
    }
  ]
})
...
const getSummaries = (param: SummaryMethodProps) => {
  ...
  columns.forEach((column, index) => {
    if (index === 0) {
      sums[index] = t('auto.views.erp.stock.out.components.StockOutItemForm.k92bcbf71')
      return
    }
