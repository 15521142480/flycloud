...
import FileOption from './component/file-option.vue'
import FileOptionModal from './component/file-option-modal.vue'
...
export default {
  ...
  methods: {
    ...
    init () {
      this.loading = true
      // this.$api.api_file.isConnectApi().then((res) => {
      //   let resultCode = res.data.code
      //   let conCode = res.data.connectCode
      //   let resultMsg = res.data.msg
      //   if (resultCode === 0) {
      //     if (conCode === '1') {
      //       this.$api.api_file.getFileListApi(this.params).then((res) => {
      //         let resultCode = res.data.code
      //         if (resultCode === "1") {
      ...
      this.$api.file.getFileListApi(this.params.path).then((res) => {
        let resultCode = res.data.code
        if (resultCode === 0) {
          this.tableData = res.data.data
        } else {
    ...
    executeCommand () {
      ...
      this.$api.file.executeCommandApi(params).then((res) => {
        this.value = ''
